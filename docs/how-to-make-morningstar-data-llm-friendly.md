...
Ask an AI assistant for a chart of a fund's sector exposure and you will usually get something that looks right. A tidy block of Highcharts configuration, plausible sector names, percentages that add up to roughly 100. Paste it into your project and you quickly discover it only looked right. The numbers were never real, and half the options do not exist in the Highcharts API.
...
**Invented API.** The model produces option names that sound like Highcharts but are not in it — a plausible-looking key on an axis, a formatting option on a series. This one is more insidious, since the config often runs, silently ignoring the keys it does not recognize.
...
Four MCP servers matter here, and they do genuinely different jobs. Three carry the workflow:
...
The fourth is **Highcharts Render**, at `https://mcp.highcharts.ai/export/mcp`, which turns a configuration into a PNG through a single `render_chart` tool, alongside optional `width`, `height` and `scale`. It is the cheapest way to see whether a chart actually looks like anything. One quirk to save you a confused minute: `render_chart` wants the config as an object, while `validate_config` takes it as a JSON string.
...
The Highcharts and DWS servers need no credentials. Morningstar uses OAuth: run `/mcp`, pick the server, and your browser opens for sign-in. The client stores the token and reuses it.
