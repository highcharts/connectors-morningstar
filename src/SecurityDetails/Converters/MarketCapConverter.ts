...
export class MarketCapConverter extends SecurityDetailsConverter {
    ...
    public override parse (
        ...
        for (const security of json) {
            ...
            const id = security.Id,
                ...
                colStrType = 'MarketCap_Type' + (isCompare ? `_${id}` : ''),
                notClassifiedStr = 'MarketCap_NotClassified' + (isCompare ? `_${id}` : ''),
                assetStr = 'MarketCap_Assets' + (isCompare ? `_${id}` : '');
            ...
            for (let i = 0; i < marketCap.length; i++) {
                ...
                const colStrAsset =
                    `MarketCap_${asset.SalePosition}` + (isCompare ? `_${id}` : '');
